...
BEGIN
  ...
  IF _status NOT IN ('cancelled','rejected') THEN RAISE EXCEPTION '취소/거절된 신청만 재신청 가능합니다'; END IF;
...
-- RLS: allow seeker to update their own application when transitioning rejected/cancelled -> pending via the SECURITY DEFINER function.
-- The function runs as definer so existing UPDATE policies are bypassed; no policy change needed.
