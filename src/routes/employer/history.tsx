...
const STATUS_LABEL: Record<string,string> = { approved:"승인", confirmed:"✅ 확정(온데요)", no_show:"노쇼" };
...
function Page() {
  ...
  useEffect(() => { if (!user) return; (async () => {
    ...
    const [jobsRes, profilesRes] = await Promise.all([
      jobIds.length ? supabase.from("jobs").select("id, title, place_name, daily_wage, work_dates").in("id", jobIds) : Promise.resolve({ data: [] as any[] }),
    ...
    setApps(arr.map((a: any) => ({ ...a, jobs: jobsMap.get(a.job_id), profiles: profilesMap.get(a.seeker_id) })));
      ...
      <div className="p-3 space-y-3">
        ...
        <Tabs value={topTab} onValueChange={(v: any) => setTopTab(v)}>
          <TabsList className="grid grid-cols-2 w-full h-12">
            <TabsTrigger value="list" className="text-base font-semibold h-10">전체 기록</TabsTrigger>
            <TabsTrigger value="calendar" className="text-base font-semibold h-10">일자별 보기</TabsTrigger>
          </TabsList>
                  ...
                  <Card key={a.id} className="p-3 mb-2 space-y-1">
                    <div className="flex justify-between items-start gap-2">
                      <div className="min-w-0">
                        <p className="font-semibold text-sm">{a.profiles?.full_name ?? "(이름미입력)"}</p>
                        <p className="text-xs text-muted-foreground">{a.jobs?.title} · {a.jobs?.place_name}</p>
                        <p className="text-xs text-muted-foreground">{new Date(a.approved_at ?? a.created_at).toLocaleString("ko-KR")}</p>
                      </div>
                      <Badge className={`text-xs px-2 py-1 ${STATUS_CLASS[a.status] ?? ""}`}>{STATUS_LABEL[a.status] ?? a.status}</Badge>
                    </div>
                  </Card>
                ))}
              </TabsContent>
          ...
          <TabsContent value="calendar" className="mt-3 space-y-3">
            ...
            <div style={{ position: "fixed", left: "-10000px", top: 0 }}>
              <PdfDoc ref={pdfRef} month={calMonth} data={confirmedByDay} employerName={employer?.full_name ?? user?.email ?? ""} />
            </div>
          </TabsContent>
        </Tabs>
      </div>
...
const PdfDoc = React.forwardRef<HTMLDivElement, { month: string; data: Map<string, any[]>; employerName: string }>(
    ...
    return (
      <div ref={ref} style={{ width: "794px", minHeight: "1123px", padding: "40px", background: "#fff", color: "#111", fontFamily: "'Malgun Gothic','Apple SD Gothic Neo','Nanum Gothic',sans-serif", boxSizing: "border-box" }}>
        ...
        <div style={{ marginBottom: "16px", fontSize: "13px", lineHeight: 1.8 }}>
          <div><strong>구인자:</strong> {employerName}</div>
